        ...
        <div x-data="{ showModal: false, selected: null }">
            <table id="preparation-table" class="border border-gray-800 shadow-lg rounded overflow-hidden mb-4 w-full">
                ...
                <tbody id="preparation-table-body">
                    @foreach($preparation_mountain as $item)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $item->user->email }}</td>
                            <td>{{ $item->mountain->name }} Via {{ $item->via }}</td>
                                @php
                                    $departure = \Carbon\Carbon::parse($item->departure_date)->translatedFormat('l, d F Y');
                                @endphp
                            <td>{{ $departure }}</td>
                            <td>
                                @php
                                ...
                                @endphp
                                <span class="px-2 py-1 rounded text-xs font-medium {{ $badgeClasses[$item->status] ?? 'bg-gray-500 text-white' }} capitalize">
                                    {{ $item->status }}
                                </span>
                            </td>
                        </tr>
