                    ...
                    @foreach ($gears as $item)
                        <tr>
                            ...
                            <td>
                                @php
                                    $badgeClasses = [
                                        'not_purchased'    => 'bg-gray-600',
                                        'in_wishlist'      => 'bg-cyan-600',
                                        'purchased'        => 'bg-emerald-600',
                                        'ready_to_use'     => 'bg-blue-600',
                                        'in_use'           => 'bg-indigo-600',
                                        'damaged'          => 'bg-red-600',
                                        'lost'             => 'bg-orange-600',
                                        'need_replacement' => 'bg-yellow-600',
                                    ];
                                @endphp
                                <span class="text-xs font-medium px-1 {{ $badgeClasses[$item->status->slug] ?? 'text-white' }} rounded">
                                    {{ $item->status->name }}
                                </span>
                            </td>
                            <td>
                            ...
                            <div class="space-y-3">
                                ...
                                <p><strong>Status:</strong>
                                    <span
                                        ...
                                        :class="{
                                            'bg-gray-600 font-medium text-xs px-1 rounded': selected.status.slug === 'not_purchased',
                                            'bg-cyan-600 font-medium text-xs px-1 rounded': selected.status.slug === 'in_wishlist',
                                            'bg-emerald-600 font-medium text-xs px-1 rounded': selected.status.slug === 'purchased',
                                            'bg-blue-600 font-medium text-xs px-1 rounded': selected.status.slug === 'ready_to_use',
                                            'bg-indigo-600 font-medium text-xs px-1 rounded': selected.status.slug === 'in_use',
                                            'bg-red-600 font-medium text-xs px-1 rounded': selected.status.slug === 'damaged',
                                            'bg-orange-600 font-medium text-xs px-1 rounded': selected.status.slug === 'lost',
                                            'bg-yellow-600 font-medium text-xs px-1 rounded': selected.status.slug === 'need_replacement',
                                        }"
                                    ></span>
                                </p>
                                <!-- Link Product -->
