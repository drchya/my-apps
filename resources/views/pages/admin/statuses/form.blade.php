...
@section('content')
    ...
    <div class="p-6 border border-gray-700 shadow-xl rounded space-y-2">
        <div class="flex items-center justify-between">
            <h6 class="text-xl">Create New Status Gear</h6>
        ...
        <form action="{{ route('setting.status.store') }}" method="POST">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 my-2">
                <div>
                    <label for="name-status" class="block text-gray-300 mb-1">Name of Status<span class="text-red-600">*</span></label>
                    <input
                        id="name-status"
                        name="name"
                        type="text"
